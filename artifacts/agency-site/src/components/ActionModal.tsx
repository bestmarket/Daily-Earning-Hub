...
const PAYMENT_METHODS = [
  { icon: <CreditCard className="w-4 h-4" />, label: "Stripe", sub: "Card / global" },
  { icon: <Wallet className="w-4 h-4" />, label: "PayPal", sub: "Global" },
...
export default function ActionModal({ isOpen, onOpenChange, toolId, toolName, isAvailable, price }: ActionModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    joinWaitlist.mutate(
      { data: { email, name, toolId, message } },
      {
        ...
        onError: () => toast({ variant: "destructive", title: "Something went wrong.", description: "Please try again later." }),
      }
    );
  };
  ...
  const handleClose = (open: boolean) => {
    onOpenChange(open);
    if (!open) setTimeout(() => { setDone(false); setEmail(""); setName(""); setMessage(""); }, 300);
  };
        ...
        {done ? (
          <div className="p-10 text-center flex flex-col items-center gap-4">
            ...
            <p className="text-muted-foreground font-medium leading-relaxed">
              {isAvailable
                ? "We'll reply within 24 hours with a payment link."
                : "You'll be the first to know when this tool launches."}
            </p>
            <Button className="rounded-full mt-2 font-bold px-8" onClick={() => handleClose(false)}>Done</Button>
          </div>
        ) : (
          <>
            <div className="bg-primary p-6 text-primary-foreground relative overflow-hidden">
              ...
              <DialogHeader className="relative z-10">
                ...
                <DialogDescription className="text-primary-foreground/80 mt-2 font-medium">
                  {isAvailable
                    ? "Leave your email and we'll send you a payment link within 24 hours."
            ...
            <div className="p-6 bg-card">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input id="name" type="text" placeholder="Jane Doe" value={name} onChange={(e) => setName(e.target.value)} className="h-11 rounded-lg bg-background" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="email" className="font-semibold">Email address *</Label>
                  <Input id="email" type="email" required placeholder="you@example.com" value={email} onChange={(e) => setEmail(e.target.value)} className="h-11 rounded-lg bg-background" />
                </div>
                {isAvailable && (
                  <div className="space-y-2">
                    <Label htmlFor="message" className="font-semibold">Any notes? (optional)</Label>
                    <Textarea id="message" placeholder="Preferred payment method, questions..." value={message} onChange={(e) => setMessage(e.target.value)} className="rounded-lg bg-background resize-none" rows={2} />
                  </div>
                )}
                <Button type="submit" className="w-full h-11 rounded-xl font-bold text-base mt-2 shadow-sm" disabled={joinWaitlist.isPending}>
                  {joinWaitlist.isPending ? <Loader2 className="w-5 h-5 animate-spin" /> : isAvailable ? "Request Access & Payment Link" : "Join Waitlist"}
                </Button>
                <p className="text-xs text-center text-muted-foreground font-medium">No spam. We'll only contact you about this tool.</p>
              </form>
            </div>
          </>
        )}
