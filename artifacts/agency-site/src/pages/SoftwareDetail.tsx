...
export default function SoftwareDetail() {
    ...
    return (
      <div className="container mx-auto px-4 py-12 max-w-5xl pt-24">
        ...
        <div className="grid md:grid-cols-3 gap-12">
          ...
          <div className="md:col-span-1"><Skeleton className="h-64 w-full rounded-2xl" /></div>
        </div>
      </div>
    );
  }
  ...
  if (!tool) {
    return (
      <div className="container mx-auto px-4 py-24 text-center pt-32">
        ...
        <Link href="/software"><Button variant="outline">Back to Catalog</Button></Link>
      </div>
    );
  }
  ...
  return (
    <>
      <SeoHead
        title={`${tool.name} | DevStudio`}
        description={tool.tagline ? `${tool.tagline} — ${tool.description?.slice(0, 120)}` : tool.description?.slice(0, 155) || ""}
        keywords={`${tool.name}, ${tool.category}, business software, DevStudio`}
        canonicalPath={`/software/${tool.id}`}
      />
      <div className="min-h-screen bg-background pt-16 pb-24">
        <div className="h-64 bg-primary/5 absolute top-0 left-0 right-0 border-b border-border/40 -z-10" />

        <div className="container mx-auto px-4 py-8 max-w-5xl mt-8">
          <Link href="/software" className="inline-flex items-center text-sm font-semibold text-muted-foreground hover:text-foreground transition-colors mb-8 group">
          ...
          <div className="grid md:grid-cols-3 gap-12 items-start">
            <div className="md:col-span-2">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <div className="flex gap-2 mb-2">
                    <Badge variant="outline" className="font-mono text-xs uppercase tracking-wider font-bold bg-background">{tool.category}</Badge>
                    {tool.featured && <Badge className="bg-accent text-accent-foreground hover:bg-accent border-transparent font-mono text-xs uppercase tracking-wider font-bold">Featured</Badge>}
                  </div>
              ...
              {tool.tagline && (
                <p className="text-2xl font-serif italic text-primary/80 mb-8 leading-snug">"{tool.tagline}"</p>
              )}

              <div className="prose prose-lg max-w-none text-muted-foreground font-medium leading-relaxed mb-12">
              ...
              <div className="mt-12 bg-muted/30 border border-border/40 rounded-2xl p-6">
                ...
                <p className="text-muted-foreground leading-relaxed text-sm">
                  {tool.name} is part of DevStudio's curated software catalog — premium tools designed for ambitious businesses. Unlike bloated SaaS subscriptions, this is a one-time purchase giving you lifetime access and direct support.
                </p>
            ...
            <div className="md:col-span-1 sticky top-24">
              <div className="bg-card border border-border/60 rounded-3xl p-6 shadow-xl shadow-primary/5 relative overflow-hidden group">
                ...
                <Button size="lg" className="w-full h-14 text-base rounded-xl font-bold shadow-md hover:shadow-lg transition-all mb-4 group/btn" variant={isAvailable ? "default" : "secondary"} onClick={() => setModalOpen(true)}>
                  {isAvailable ? "Get Access Now" : "Join the Waitlist"}
                  <ArrowRight className="w-5 h-5 ml-2 group-hover/btn:translate-x-1 transition-transform" />
                </Button>
                <Button variant="outline" size="sm" className="w-full h-10 text-sm rounded-xl font-semibold gap-2 transition-all" onClick={handleShare}>
                  {copied ? <><Check className="w-4 h-4 text-green-500" /><span className="text-green-600">Link Copied!</span></> : <><Share2 className="w-4 h-4" />Share this Tool</>}
                </Button>
                <div className="space-y-3 pt-4 border-t border-border/40 mt-6">
                  <div className="flex items-center gap-2 text-xs font-semibold text-muted-foreground">
                    <ShieldCheck className="w-4 h-4 text-green-500" /> Secure payment via Stripe / PayPal
                  </div>
                  <div className="flex items-center gap-2 text-xs font-semibold text-muted-foreground">
        ...
        <ActionModal isOpen={modalOpen} onOpenChange={setModalOpen} toolId={tool.id} toolName={tool.name} isAvailable={isAvailable} price={tool.price} />
      </div>
    </>
  );
}
